...
:Автор karate-coq: Рейнальд Аффельдт (Reynald Affeldt)
:Дата публикации karate-coq: 13 января 2025 г.
:Автор курса по Coq: Антон Трунов
:Дата публикации оригинала курса по Сoq: 11 марта 2021 г.

:Автор вот этой хуйни: Вася
...
ещё к психологу хожу, лайфкоучу. Вчера ещё у таролога был. |*)
...
(*| Тут получаем предупреждение, тк мы объявили тип `void` во вселенной типов
(`Type`), но петушара автоматически понизил его до `Prop` (логических
утверждений). Такое автоматическое поведение с понижением до `Prop` — устаревшее
и скоро станет невозможным.

- Coq различает два главных юниверса: `Type` (обычные типы: множества, структуры)
  и `Prop` (логические утверждения — истинные или ложные).​

- В старых версиях Coq, если ты объявляешь `void` как тип в `Type`, но используешь
  его там, где ожидается `Prop`, Coq автоматически переводит его в `Prop` (это
  называется "prop lowering").​

- Текущий warning говорит: "Я объявлен в `Type`, но меня используют как `Prop` →
  делаю автоматическое понижение". Это скоро перестанет делаться автоматически,
  чтоб избежать неявного поведения.
|*)

(*| Из пустоты (неконструируемого) можно получить всё, но сначала *предъяви
пустоту* (неконструироемое). |*)
...
(*| .. admonition:: Зависимая функция.

       `forall x : A,B` называется **зависимо типизированной
...
<https://inria.hal.science/hal-01094195/document>`_, а пока нас это ебать не
должно. |*)
...
(житель/значение типа), и значит `*` — это нотация не для `Nat.muln`, а для
`prod`. |*)
...
(*| .. admonition:: erefl.
       ...
       Нам пока не объяснили, что значит `erefl`. Пока просто поверим,
       что если эти строки выше проходят проверку типов, то равенства верны.
|*)
...
(*| Нам нужно построить свою собственную логику, в которой мы будем выражать
обычные связки вроде "и", "или", "не" и т.д, а также кванторы, используя стиль
`натуральной дедукции
<https://ru.wikipedia.org/wiki/%D0%9D%D0%B0%D1%82%D1%83%D1%80%D0%B0%D0%BB%D1%8C%D0%BD%D1%8B%D0%B9_%D0%B2%D1%8B%D0%B2%D0%BE%D0%B4>`_.

Логика, которую мы собираемся эмулировать в теории типов, классифицируется как
`интуиционистская логика
<https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D1%82%D1%83%D0%B8%D1%86%D0%B8%D0%BE%D0%BD%D0%B8%D1%81%D1%82%D1%81%D0%BA%D0%B0%D1%8F_%D0%BB%D0%BE%D0%B3%D0%B8%D0%BA%D0%B0>`_
высшего порядка. Считается, что отцом `интуиционизма
<https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D1%82%D1%83%D0%B8%D1%86%D0%B8%D0%BE%D0%BD%D0%B8%D0%B7%D0%BC>`_
считается `Брауэр
<https://ru.wikipedia.org/wiki/%D0%91%D1%80%D0%B0%D1%83%D1%8D%D1%80,_%D0%9B%D1%91%D0%B9%D1%82%D0%B7%D0%B5%D0%BD_%D0%AD%D0%B3%D0%B1%D0%B5%D1%80%D1%82_%D0%AF%D0%BD>`_.
С 1904 года Брауэр последовательно проводил критику так называемых чистых
математических доказательств существования, опирающихся на логический принцип
исключённого третьего, что в конечном счёте положило начало целому направлению в
обоснованиях математики математическому интуиционизму.

Антон в одной из свох лекций рассказывал, что в то время Брауер заметил, что
многие современные ему математики не предъявляют конкретных конструкций. В
частности Гильберт популяризовал доказательства от противного. Ну типа
предполагаем, что что-то не существует, выводим противоречие, снимает двойное
отрицание и мы доказали существование объекта без его предъявления.

Иногда люди используют слова "конструктивисткая" и "интуиционистская" как
синонимы. Строго говоря, это не совсем корректно, но мы не будем душнить.

В конструктивной логике существование математического объекта доказывается с
помощью метода (или алгоритма), который реально этот объект строит. В теории
типов такие методы выражаются как термы.

Между термами и типами с одной стороны, и доказательствами и высказываниями — с
другой, существует соответствие. Теория типов позволяет не только эмулировать
логику высшего порядка, но и оперировать самими доказательствами как
полноценными объектами: передавать их в функции, упаковывать в структуры данных,
возвращать из функций и так далее. Это делает из теории типов мощный инструмент
для математических рассуждений и верификации программ. |*)
...
(*| Импликация соответствует типу функции. Доказательство утверждения "A влечет B" — это
просто функция типа `A -> B`, которая преобразует доказательство `A` в
доказательство `B`.
...
(*| Это просто тождественная функция, как мы уже видели ранее и её можно
понимать как *лемму* под названием `a_implies_a`. |*)

(*| Мы используем вселенную `Prop` — это примитив Coq, о котором будет
рассказано чуть ниже. Сейчас достаточно знать, что `A : Prop` значит "A — это
высказывание". |*)
...
(*| А `modus ponens <https://ru.wikipedia.org/wiki/Modus_ponens>`_ - это
просто применение функции: |*)
...
(*| Конструктивное доказательство конъюнкции :math:`A \land B` — это пара
доказательств `A` и `B`: |*)
...
(*| Это доказательство выглядит точь‑в‑точь как функция, переставляющая местами
компоненты пары. |*)
...
(*| Конструктивное доказательство дизъюнкции :math:`A \lor B` — это либо
доказательство `A`, либо доказательство `B`, плюс "тэг/метка", показывающая, какое
именно из высказываний доказано. Короче, определение дизъюнкции выглядит так:
|*)
...
(*| Мы там выше уже немного потыкали неявные аргументы, но вот таких штук пока
не встречали, поэтому надо бы разобраться: |*)

  Arguments or_introl [A] B _, [A B] _.
  Arguments or_intror [A] B _, [A B] _.

(*| Тут `Arguments` указывает на то, какие есть варианты указать аргументы. В
каком случае какие аргументы должны выводиться неявно, а какие указываться явно.
...
`[A]` можно вывести неявно потому, что доказательство `or_introl of A` уже имеет
тип `A`. Ну просто потому что все конструкторы наследуют параметры самого
индуктивного типа, который они конструируют: |*)

  Check or_introl. (* .unfold *)

(*|

**Вариант [A B] _**

Иногда `B` может быть выведен из контекста (ниже будет пример такого случая).
Так вот в варианте `[A B] _` мы говорим, что если `or_introl` применён к всего лишь одному аргументу, то нужно
вывести из контекста оба утверждения: `A` и `B` (если не получится это сделать, то будет ошибка).
...
(*| А вот так будет выглядеть конструктивной логике доказательство
дистрибутивности конъюнкции (логического "и") над дизъюнкцией (логическим "или")

:math:`(A \lor B) \land C \to (A \land C) \lor (B \land C)`
...
(*| Ну смысл понятен, да? Мы просто конструируем штуки всякие. Поэтому и логика
такая называется *"конструктивная"*. А как мы их конструируем? - Мы пишем функции.
|*)
...
(*| Тривиально истинное высказывание не несет никакой информации, и его
доказательство тоже должно быть тривиальным. Поэтому истинное высказывание —
это, по сути, юнит-тип во вселенной `Prop`. У него ровно один конструктор с
историческим именем `I`. |*)
...
(*| Так как у `False` нет конструкторов, его нельзя доказать без дополнительных
предположений — то есть в пустом контексте, если только в самом Coq нет
критических багов (а они иногда бывают). Команда разработчиков даже держит
`список таких багов
<https://github.com/coq/coq/blob/master/dev/doc/critical-bugs](https://github.com/coq/coq/blob/master/dev/doc/critical-bugs>`_.

Поскольку у `False` нет конструкторов, выражение сопоставления с образцом для
него имеет интересную форму: ветвей нет, а значит, можно построить терм любого
типа, *потому что ничего определять не нужно*. Это известный **принцип взрыва** (ex
falso quodlibet) — из лжи следует всё. Вот пример: |*)
...
(*| Логика, построенная таким образом, называется интуиционистской. В ней, как
известно, нельзя в общем случае доказать классический принцип устранения
двойного отрицания (DNE - double negation elimination), то есть нельзя построить
терм типа `~~A -> A` для произвольного `A`: |*)
...
(*| Всё что у нас есть это терм вот такого типа `not_not_a: (A -> False) ->
False`. Это ф-ция, но мы её "вызвать" не можем, тк у нас нет ни `A` ни
`(A -> False)`.
...
(*| Без сахара это `A \/ (A -> False)`. Чтобы доказать дизъюнкцию нам нужно
доказать правый либо левый дизъюнкт, а мы не можем ни то ни другое
сконструировать. |*)
...
(*| Как и отрицание, логическая эквивалентность не является first-class
коннектором в Coq. Вместо этого есть определение `iff`, которое означает
конъюнкцию двух импликаций + нотация `<->`. |*)
...
(*| Доказательство высказывания :math:`\forall x. P(x)` — это функция, которая
принимает какой-то конкретный :math:`x` и возвращает доказательство :math:`P(x)`
и тип вот этого доказательства зависит от `x`.

Ну то есть у нас есть некий :math:`x` и утверждение :math:`P(x)`, тип которого
меняется в зависимости от :math:`x`. Это значит, что :math:`P(x_1)` и
:math:`P(x_2)` имеют разные типы. Ну а это ровно то, чем является `зависимая
функция`_ в теории типов.

Здесь мы не можем сами определить этот тип зависимых функций (квантор
:math:`\forall`), потому что это примитив Coq.
...
(*| .. admonition:: Квантор существования.

       Это тип *зависимых пар*.

       Конструктивно доказательство :math:`\exists x. P(x)` состоит из двух компонентов:

       1. Конкретного элемента :math:`x`
       2. Доказательства того, что :math:`P(x)` выполняется для этого элемента :math:`x`
...
(*| Получаем тип, обобщающий конъюнкцию (:math:`\land`), только теперь *тип*
второго компонента зависит от *значения* первого. |*)
...
1. Нотация `exists x y, p` — это сокращённо от `ex (fun x => ex (fun y => p))`,
т.е. вложенное существование: :math:`\exists x`, :math:`\exists y`, и при этом
:math:`p` истинно.

2. Команда `binder x` — позволяет коку понять, что `x` это переменная-байндер. Так он
сможет распарсить такой список переменных подряд: `x .. y` не просто как список
символов, а как цепочку связанных переменных.

3. `format` — отвечает за претти-принтинг. Позволяет нам напечатать выражение
`exists x y, p` вот так:
...
То есть символы `'['` и `']'` это просто визуальное оформление, `/` — перевод
строки с отступом, а сама запись становится читабельнее в доказательствах.
...
(*| Чтобы легко парсить в голове утверждения, в которых присутствует ложь, важно
помнить чем является ложь в конструктивной логике:

  `~ A` :math:`\equiv` `A -> False`
...
Мы ему дали конкретный :math:`x` для которого выполняется :math:`P(x)` и само
:math:`P`. |*) 
...
- `P` является **зависимым предикатом** (функцией из элементов типа `A` в
  `Prop`), и его можно явно передавать в `ex_into`, либо использовать `_`, чтобы
  кок его вывел за нас


|*) 

(*| .. admonition:: Зависимый предикат.

       Это тип функции вида `P : A -> Prop`, которая для каждого элемента `x` из
       типа `A` выдает утверждение `Prop` о нем. |*)
...
(*| Одна из центральных тем теории типов. Там существует целая иерархия понятий
равенства или эквивалентности. |*)
...
(*| На мета-уровне существует *встроенное* понятие равенства между термами. Оно
утверждает, что любые два терма равны, если можно свести один к другому
вычислениями (т.е. если они "конвертируемы"). Такое равенство нельзя доказать,
такие равенства - внутренняя механика кока. То есть, насколько я понял, кок
просто запускает вычисление, смотрит на редуцированную пару термов и заключает
"равны" они или нет. |*)
...
(*| Это и называется **пропозициональное равенство**. Единственное свойство,
заложенное в этом равенстве — *рефлексивность*.

Это первая встреча с *индексированными* типами и надо бы разобраться.

В этом определении тип `A` после двоеточия называется *индексом*, а `x` — *параметром*.
Параметры должны быть одинаковыми для всех конструкторов, а индексы могут
меняться. В данном случае конструктор один, поэтому вариантов нет. Но именно
благодаря такому механизму паттерн-матчинг позволяет симулировать
равенство между термами. |*)
