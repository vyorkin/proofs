...
(*| Не знаю правильно ли я перевёл, в оригинале это definitional equality
(дефиниционное равенство?). |*)
...
вычислениями (т.е. конвертируемы). Важно понимать, что нельзя доказать
такое равенство, тк такие равенства - внутренняя механика кока. |*)
...
(*| Мы можем ввести пропозициональное равенство. Это будет первая встреча с
*индексированными* типами. |*)

  Inductive eq (A : Type) (x : A) : A -> Prop :=
  | eq_refl : eq x x.
  
(*| Единственное свойство, заложенное в этом равенстве — *рефлексивность*.

В этом определении тип `A` после двоеточия называется *индексом*. А `x` —
*параметром*. Параметры должны быть одинаковыми для всех конструкторов, а
индексы могут меняться. В данном случае конструктор один, поэтому вариантов нет.
Но именно благодаря такому механизму сопоставление с образцом позволяет
симулировать равенство между термами. |*)
